...
/**
 * Mobile LocalSense source ranking.
 * Known deterministic phone state always outranks optional OCR/vision/model interpretation.
 */
object RavenLocalSenseOS {
    ...
    fun resolve(marker: RavenMarkerBus.Marker): Decision {
        ...
        return when {
            ...
            key == "NOTIFICATION_POSTED" ->
                Decision(Route.NOTIFICATION_SOURCE, true, reason = "notification-source metadata")
            key.startsWith("MEDIA") ->
                Decision(Route.MEDIA_SESSION, true, reason = "media semantic state")
            key.startsWith("BATTERY") || key.startsWith("POWER") || key.contains("SCREEN") ->
